...
| Tab 2 | 我的 | 个人主页（本人视角）、档案馆、我的创建/申请/足迹 |
...
| pages/profile/index | 用户主页 | 他人访问时触发 | 本人=完整态，他人=只读精简态；通过 uid 参数区分 |
| pages/profile/edit | 编辑个人资料 | 是 | 昵称/性别/身份 tag/签名/QQ 号 |
| pages/profile/log_detail | 演绎记录详情 | 否（只读）/ 是（编辑） | 本人右上角编辑按钮切换编辑态 |
| pages/mine/index | 我的（Tab2） | 是 | Tab2 壳页面，内部渲染与 profile/index 本人视角完全相同的内容；不独立维护 UI 逻辑 |
| pages/mine/created | 我的创建/申请/足迹 | 是 | tab 参数区分三视图 |
| pages/mine/detail | 发布人详情页 | 是 | 从「我的创建」点卡片进入 |
| pages/mine/review | 审核页 | 是 | 发布人专属，查看申请人列表并审批 |

> ⚠️ `pages/auth/login` 已废弃，不再使用独立登录页，改为授权弹窗。
>
> ⚠️ `pages/mine/index` 作为 Tab2 的壳页面存在（因微信 TabBar 页面不支持传参），其内部逻辑与 `pages/profile/index` 本人视角共用同一套组件，不单独维护 UI。
...
| 已登录他人 · 未申请 | recruiting | 申请加入 | 深色宽按钮 | 提交申请（先做状态检查） |
...
| 发布 | 深色主按钮 | 弹提示「发出去喽」确认，以 recruiting 状态存入 store，跳转「我的创建」 |

#### 返回键行为（新建模式）

| 情况 | 返回键触发 | 弹窗内容 | 按钮一 | 按钮二 |
|---|---|---|---|---|
| 模组名称已填 | 弹确认窗 | 「是否保存为草稿？」 | 保存草稿（等效底部按钮，跳我的创建） | 不保存（直接返回，不生成卡片） |
...
*最后更新：2026 年 3 月 24 日 | 版本：V1.2 架构复盘修订版（基于 prd032401）*
